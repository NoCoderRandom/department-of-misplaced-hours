...
- `scripts/qa-playthrough.mjs` - automated browser QA for ship checks, including all authored hotspot cursor/status coverage, route-aware wording, rain-note opt-out, modal backdrop click shielding, final-choice ending matrix coverage, Credits source-document URL targets, ending Credits/actions after reload, and late-game spent-item cleanup.
