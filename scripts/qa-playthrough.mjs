...
async function touchTap(page, x, y, wait = 180) {
  const point = await page.evaluate(
    ({ gameX, gameY, gameW, gameH }) => {
      const canvas = document.querySelector("canvas");
      if (!canvas) {
        throw new Error("Canvas not found for game-coordinate touch.");
      }
      const rect = canvas.getBoundingClientRect();
      return {
        x: rect.left + (gameX / gameW) * rect.width,
...
async function testPuzzlePolish(browser, issues) {
  ...
  let spentPaperworkData = await save(page);
  if (spentPaperworkData.inventory.includes("blankForm")) {
    throw new Error(`Stamped paperwork let In-Tray reissue a spent blank form: ${JSON.stringify(spentPaperworkData)}`);
  }
  await button(page, "Close");
  await click(page, 590, 660);
  await page.getByText("spent its authority on Form 11-H").waitFor({ state: "visible", timeout: 8_000 });
  spentPaperworkData = await save(page);
  if (spentPaperworkData.inventory.includes("rubberStamp")) {
    throw new Error(`Stamped paperwork let Rubber Stamp reissue a spent stamp: ${JSON.stringify(spentPaperworkData)}`);
  }
  await button(page, "Close");
  ...
  for (let hintPress = 0; hintPress < 3; hintPress += 1) {
    ...
    if (hintPress === 2) {
      ...
      if (!hintText.includes("rebuilt from first, middle, and last clues")) {
        throw new Error(`Final clock hint did not give a non-spoiler nudge: ${hintText}`);
      }
      if (hintText.includes("Regret, Hunger, Calm, Joy")) {
        throw new Error(`Final clock hint revealed the answer without confirmation: ${hintText}`);
      }
  ...
  const spentIngredientState = {
    inventory: ["memoryCup", "serverFuse", "selfFile"],
    flags: {
      introSeen: true,
      formStamped: true,
      clockUnlocked: true,
  ...
  await continueSaved(page, { ...spentIngredientState, room: "break" });
  await click(page, 398, 606);
  await page.getByText("none of them volunteer").waitFor({ state: "visible", timeout: 8_000 });
  let spentData = await save(page);
  if (spentData.inventory.includes("paperCup")) {
    throw new Error(`Solved vending let Paper Cups reissue a spent paper cup: ${JSON.stringify(spentData)}`);
  }
  await button(page, "Close");
  await continueSaved(page, { ...spentIngredientState, room: "archive" });
  await click(page, 910, 558);
  await page.getByText("refuses to mint another Time Token").waitFor({ state: "visible", timeout: 8_000 });
  spentData = await save(page);
  if (spentData.inventory.includes("timeToken")) {
    throw new Error(`Solved vending let Coin Drawer reissue a spent time token: ${JSON.stringify(spentData)}`);
  }
  await button(page, "Close");
...
async function testScaledInteraction(browser, issues) {
  ...
  watchPage(touchPage, issues, "touch-hotspot-prime");
  try {
    ...
    mobileData = await save(touchPage);
    if (!mobileData.inventory.includes("blankForm")) {
      throw new Error(`Touch second tap did not collect the blank form: ${JSON.stringify(mobileData)}`);
    }

    await touchTap(touchPage, 590, 660);
    mobileData = await save(touchPage);
    if (mobileData.inventory.includes("rubberStamp")) {
      throw new Error(`Touch first tap activated instead of previewing the stamp: ${JSON.stringify(mobileData)}`);
    }
    await touchTap(touchPage, 590, 660);
    mobileData = await save(touchPage);
    if (!mobileData.inventory.includes("rubberStamp")) {
      throw new Error(`Touch second tap did not collect the rubber stamp: ${JSON.stringify(mobileData)}`);
    }

    await selectItem(touchPage, "blankForm");
    await touchTap(touchPage, 590, 660);
    await button(touchPage, "Accept");
    mobileData = await save(touchPage);
    if (!mobileData.inventory.includes("stampedForm") || mobileData.inventory.includes("blankForm") || mobileData.inventory.includes("rubberStamp")) {
      throw new Error(`Selected inventory did not bypass touch preview for immediate use: ${JSON.stringify(mobileData)}`);
    }
  } finally {
