...
- `scripts/smoke-release-archives.mjs` extracts both pending ZIP formats, serves the actual web roots, verifies Credits source-document targets, verifies controller-B Clock-In gating, starts a new shift, verifies the no-JavaScript fallback, and verifies touch-phone portrait orientation gating before release artifacts are promoted.
- `scripts/smoke-live.mjs` checks the deployed GitHub Pages URL, including the live runtime JS/CSS chunk manifest and size budgets, static metadata, install metadata, app icons, social card, robots/sitemap, Credits source-document targets, canvas accessibility, fresh-shift boot with controller-B Clock-In gating, no-JavaScript fallback, and touch-phone portrait orientation gating. Its success JSON reports the validated manifest fields, app-icon dimensions, social-card dimensions, crawler files, and runtime chunk byte sizes so release evidence is visible without opening the script.
