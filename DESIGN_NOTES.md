...
- `scripts/qa-playthrough.mjs` can run quick development QA or production-preview QA; the release gate verifies the built `dist/` through asset-load failure recovery with DOM alert text, optional audio fallback, intro Clock-In gating with legacy badge recovery, title/help Credits access, dialog title/body accessibility semantics, live keyboard target status, phone portrait rotate gating, puzzle-polish checks for Notes/objectives/side-room clue recall/hint answer reveal/Auditor feedback/route wording/rain-note opt-out, both main routes plus the third audit ending, final-choice ending matrix coverage with Play Again reset, canvas paint checks, rendered mobile canvas fit/aspect checks, mid-game reloads, late-game reloads after Auditor verification and server unlock, non-spoiler phone clue recall in Notes and puzzle clue review, typed and clicked vending keypad paths, keyboard/gamepad puzzle-modal solving, phone/rain/muted clue paths with immediate muted phone/tape transcripts, hand-cursor hotspot/inventory behavior, touch first-tap hotspot preview with immediate selected-item use, sequence puzzle undo/backspace recovery, selection-safe audio controls, keyboard shortcuts, selected-item cancel by Escape/right-click/controller B, right-click panel close, modal backdrop click shielding, keyboard title start, protected Start New behavior, controller title/stick/object/modal navigation plus hint and bumper controls, clue-gated Mood Clocks, large-text and reduced-motion preference persistence/layout/reset survival, system reduced-motion migration for legacy saves, keyboard object/inventory interaction, early and late-game wrong-item feedback, Auditor consultation notes and hour-presentation recovery, failed-puzzle recovery, reward Escape checks for rain/glass/vending note prompts plus vending reward reload recovery, save repair, invalid-room save recovery, corrupt/unavailable storage recovery with player warning, Recover Position, archive gates, pre-file vending story gate, malformed save normalization, scaled desktop/mobile interaction, modal focus/Escape behavior, reset behavior, late-game Notes scrolling, answer-order anti-spoiler checks, downstream save repair, and desktop/mobile visual readability screenshots including Credits panels, Future Phone story panels, and ending screens.
...
- `scripts/smoke-release-archives.mjs` extracts both pending ZIP formats, serves the actual web roots, verifies Credits source-document targets, verifies controller-B Clock-In gating and right-click panel close, starts a new shift, verifies the no-JavaScript fallback, and verifies touch-phone portrait orientation gating before release artifacts are promoted.
- `scripts/smoke-live.mjs` checks the deployed GitHub Pages URL, including the live runtime JS/CSS chunk manifest and size budgets, static metadata, install metadata, app icons, social card, robots/sitemap, Credits source-document targets, canvas accessibility, fresh-shift boot with controller-B Clock-In gating and right-click panel close, no-JavaScript fallback, and touch-phone portrait orientation gating. Its success JSON reports the validated manifest fields, app-icon dimensions, social-card dimensions, crawler files, and runtime chunk byte sizes so release evidence is visible without opening the script.
...
- Standard controller/gamepad input reuses the same focus targets: D-pad or left stick cycles title actions, ending actions, room objects, inventory, and modal buttons; A selects; B cancels selected items or closes ordinary panels; required one-button gates such as Clock In wait for explicit confirmation; face/menu buttons open common panels.
...
- The phone/tape audio clue is remembered in Notes and Memory Vending review as non-spoiler recall after discovery; exact digits come from seeing the rain visual clue, the muted accessibility transcript, or explicit Hint answer. Keeping the Rain Cipher note is an optional inventory reminder, and Notes records the solved code after Memory Vending is complete for final-act recall.
...
- Hints keep exact sequence, code, and final-answer solutions behind an explicit Show Answer step, so stuck players can still recover without accidental spoilers.
