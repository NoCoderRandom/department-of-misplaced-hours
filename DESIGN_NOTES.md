...
- `scripts/check-release.mjs` validates the exact expected build asset manifest, app icons, production CSP hardening, image provenance hashes, blocks extra `dist/` files, sourcemaps, unapproved payload regressions, oversized `dist/`, and accidental package contents such as source, temp files, backups, prompts, or duplicate public assets.
